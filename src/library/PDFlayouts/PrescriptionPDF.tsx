...
const PrescriptionPDF: React.FC<PrescriptionPropsWithQR> = ({
  drugs,
  patientData,
  userData,
  qr,
      ...
      <Page size="A4" style={styles.page}>
          ...
          <View style={styles.rightColumn}>
            <Text style={[styles.dateText, styles.poppins]}>
              ...
              {patientData.LUID === "synthesized-temporal-patient"
                ? CurrentDate()
                : `${MongoDbIdDateRetriever(drugs._id)}`}
            </Text>
          </View>
        </View>
        ...
        <View style={styles.section}>
          ...
          {drugs.type === "processed"
            ? drugs.values.map((drug, index) => {
                return (
                  <Text
                    style={[styles.drugItem, styles.poppins]}
                    key={index}
                  >{`• ${drug.name} — ${drug.indication}`}</Text>
                );
              })
            : drugs.values.map((drug, index) => {
                return (
                  <Text
                    style={[styles.drugItem, styles.poppins]}
                    key={index}
                  >{`• ${drug.drugPresentation?.includes("~") ? drug.drugPresentation.split("~")[0].trim() : drug.drugPresentation}; ${drug.indication}`}</Text>
                );
              })}
        </View>
        ...
        {/* ----------- Cut Section (Resumen para desprender) ----------- */}
        <View style={styles.cutSection}>
          {/* Left Column - Medicamentos */}
          <View style={styles.cutLeftColumn}>
            <Text style={[styles.cutTitle, styles.poppins]}>Medicamentos</Text>

            {drugs.type === "processed"
              ? drugs.values.map((drug, index) => (
                  <Text
                    key={`cut-${index}`}
                    style={[styles.poppins, styles.cutInfoText]}
                  >
              ...
              : drugs.values.map((drug, index) => (
                  <Text
                    ...
                    style={[styles.poppins, styles.cutInfoText]}
                  >
                    • {drug.drugPresentation}
                  </Text>
                ))}
          </View>

          {/* Right Column - Información General */}
          <View style={styles.cutRightColumn}>
            <Text style={[styles.poppins, styles.cutInfoText]}>
              Fecha:{" "}
              {patientData.LUID === "synthesized-temporal-patient"
                ? CurrentDate()
                : `${MongoDbIdDateRetriever(drugs._id)}`}
            ...
            <Text
              style={[styles.poppins, styles.cutInfoText, { marginBottom: 40 }]}
            >
              Paciente: {patientData.personalInfo.names}{" "}
              {patientData.personalInfo.middleName}{" "}
            ...
            >
              <Text style={[styles.poppins, { marginLeft: 40 }]}>
