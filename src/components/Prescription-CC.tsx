...
import PdfViewerCC from "./PdfViewer-CC";

const PrescriptionCC: React.FC<PrescriptionProps> = (props) => {
  const { drugs, patientData } = props;
  const [qrCode, setQrCode] = useState<string>("");
  const { setGlobalModal } = useGlobalContext();
  const datePrescribed = useMemo(() => {
    return MongoDbIdDateRetriever(drugs._id);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  const sharePDF = () => {
    navigator.clipboard.writeText(
      `${process.env.NEXT_PUBLIC_NOT_FRONTEND_URL}/shared/Prescription/${drugs._id}`,
    );
    toast.success("Link a la receta electronica copiada al Portapapeles");
  };
  ...
  return (
    <div className="PrescriptionCC">
      <h1 className="title">{"Rx"}</h1>
      <p className="date">{datePrescribed}</p>
      <div className="drugContainer">
        {drugs.values
          ? drugs.type === "local"
            ? drugs.values.map((drug) => {
                return (
                  <div
                    className="drugLine"
                    key={`${drug.drugPresentation} ${drug.indication}`}
                  >
                    <p>{drug.drugPresentation}</p>
                    <p>{drug.indication}</p>
                  </div>
                );
              })
      ...
      <div className="buttonContainer">
        ...
        <ButtonCC
          ...
          type="Phantom"
          onClick={() => PrintPDFPrescription(props, qrCode)}
        />
        <ButtonCC
          icon={
            <svg
