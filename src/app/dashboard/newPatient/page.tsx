...
const NewPatientIndex: React.FC = () => {
  ...
  // ── Arranque del flujo ─────────────────────────────────────────────────────
  // Esta ruta es solo el alta de paciente; el flujo de documento vive en
  // /dashboard/addDocument con su propia página. No se resetea si ya se venía a
  // media captura, para que volver atrás no borre todo.
  useEffect(() => {
  ...
  const AddPatientFlow: Array<React.ReactElement> = [
    ...
    <div className="formOptContainer" key={"ManualIdFormContainer"}>
      <FormCC
        ...
        onSubmit={(formData) => {
          ...
          if (!["Masculino", "Femenino", "Intersexual"].includes(selectedSex)) {
          ...
          const patientIdentification: PatientIdentification = {
            ...
            birthPlace: [
              ...
              .filter(Boolean)
              .join(", "),
            nacimientoLocalizacion: manualBirthLocalizacion as LocalizacionItem,
  ...
  return (
    <section className="NewPatientContainer">
      {currentSessionData.currentRecord ? (
        ...
        (AddPatientFlow[currentSessionData.currentStep] ?? AddPatientFlow[0])
      )}
    </section>
  );
};
